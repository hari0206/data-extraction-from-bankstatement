...
def get_results():
    ...
    file_ = request.files["images"]
    filename = secure_filename(file_.filename)
    file_path=os.path.join(os.path.join(os.getcwd(),"files"),filename)
    file_.save(file_path)
    result=extract_data(bank_name,file_path)
    result = result["data"]
    # print(result)
...
def match_headers(headers,data):
    ...
    header_list=[header for header in header_list_bank if len(header.strip())>0]
